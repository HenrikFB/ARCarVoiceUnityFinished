...
public partial class Wit3D : MonoBehaviour {
    ...
    public bool[,] itemOptions = new bool[,]{
 	//options order =open door, close door, start engine, stop engine, colour, windows, show engine, video, bonnet, hood, trunk
					...
					{true,true,true,true,true,false,true,true,true,true,false}, //Police Car
                        ...
                        switch (aPart.value)
                        {
                            ...
                            case "video":
                                if (errorCheck(7))
                                {
                                    if (theAction._text.Contains("video"))
                                    {
                                        GameObject.Find(colourSwitcher.instance.getCurrentTracked().name + "/activeItems/" + gameController.currentSelectedCar + "/video").GetComponent<VideoPlayer>().Pause();
                                    }
                                }
                                else
                                {
